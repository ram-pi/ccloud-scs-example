...
public class CcloudApplication {
	...
	public Supplier<Message<Sensor>> generate() {
		return () -> {
			...
			sensor.setAccelerometer(Collections.singletonList((float) faker.number().numberBetween(1, 100)));
			sensor.setMagneticField(Collections.singletonList((float) faker.number().numberBetween(1, 100)));
